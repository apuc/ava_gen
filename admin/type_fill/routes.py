...
type_fill_page = Blueprint('type_fill', __name__, template_folder='templates')


@type_fill_page.route('/type_fill', defaults={'current_page': 1})
@type_fill_page.route('/type_fill/<current_page>')
def index(current_page):
    ...
    return render_template('admin/type_fill/index.html', crud=crud)


@type_fill_page.route('/type_fill/create', methods=['GET'])
def create():
    type_fill_model = Type_Fill()
    types = TypeService.find()
    fills = FillService.find()
    return render_template('admin/type_fill/form.html', type_fill=type_fill_model, types=types, fills=fills)


@type_fill_page.route('/type_fill/remove/<id>')
def remove(id):
    Type_FillService.remove(id)

    flash('Запись удалена')
    return redirect(url_for('type_fill.index'))


@type_fill_page.route('/type_fill/edit/<id>')
def edit(id):
    ...
    return render_template('admin/type_fill/form.html', type_fill=type_fill_model, types=types, select_type=select_type, fills=fills, select_fill=select_fill)


@type_fill_page.route('/type_fill/save', methods=['POST'])
def save():
